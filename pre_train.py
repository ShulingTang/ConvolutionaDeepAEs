...
if __name__ == "__main__":
    ...
    channels_all = [[1, 50], [1, 3, 5], [1, 15, 15], [1, 15], [1, 3, 3], [1, 15, 5], [1, 3, 3, 5]]
    kernels = [3, 3]
    epoches = 40
    lr = 1e-2
    weight_decay = 1e-5
    for item in channels_all:
        channels = item
        finename = 'pre_train_result'
        with open(finename, 'a') as f:
            f.write('channels:%s,kernels:%s\n' % (channels, kernels))
        ...
        for epoch in range(epoches):
            ...
            finename = 'pre_train_result'
            with open(finename, 'a') as f:
                f.write("epoch: {}, loss is {}".format((epoch + 1), loss.data))
        torch.save(model.state_dict(), './Mnist-%s.pkl' % channels)
